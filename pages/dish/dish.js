...
Page({
 ...
 async loadComments(){try{let voteMap=this._dishVotesByUser||{};if(!Object.keys(voteMap).length&&wx.cloud&&wx.cloud.callFunction){const r=await wx.cloud.callFunction({name:'dishHobbyReviews',data:{dishId:this.data.dish.id}});voteMap=(r&&r.result&&r.result.data&&r.result.data.votesByUser)||{};}
  const r=await call('commentsTree',{dishId:this.data.dish.id,page:1}),roots=Array.isArray(r&&r.data)?r.data:[];
  const comments=roots.map(item=>Object.assign({},item,{vote:voteMap[item.openid]||'none',replies:(item.replies||[]).map(v=>Object.assign({},v,{vote:voteMap[v.openid]||'none'}))}));this.setData({comments})
