Page({
 ...
 buildSummary(){const g=this.data.group;if(!g)return '';const active=(g.members||[]).filter(x=>Number(x.qty)>0);const lines=active.map(x=>`${x.name||'匿名吃货'}    ${Number(x.qty)}${g.unit||'份'}`);const total=Number(g.price||0)*(Number(g.totalQty)||0);return[`【${g.title||'团购'}】`,`商品：${g.dishName||''}`,g.storeName?`店铺：${g.storeName}`:'',...lines,`合计：${Number(g.totalQty)||0}${g.unit||'份'} / ¥${total}`].filter(Boolean).join('\n')},
 copySummary(){const text=this.buildSummary();this.setData({summaryText:text});wx.setClipboardData({data:text,success:()=>wx.showToast({title:'统计清单已复制',icon:'success'})})},
