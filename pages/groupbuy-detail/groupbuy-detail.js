Page({
 ...
 load(){wx.cloud.callFunction({name:'groupbuy',data:{action:'detail',id:this.id}}).then(r=>{const d=r.result&&r.result.data;if(!d)return wx.showToast({title:(r.result&&r.result.message)||'加载失败',icon:'none'});this.setData({group:d,name:d.mine&&d.mine.name||'',qty:d.mine&&d.mine.qty||1,note:d.mine&&d.mine.note||'',totalAmount:(d.price||0)*(d.totalQty||0),isOwner:d.creatorId===d.mine?.openid})})},
