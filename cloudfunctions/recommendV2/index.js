...
exports.main=async(e)=>{
 ...
 try{
  ...
  const [dishesRes,actionsRes,eatRes,favRes,eventsRes,allActionsRes]=await Promise.all([
   ...
   db.collection('user_action_events').where({openid:me}).orderBy('createdAt','desc').limit(2000).get().catch(()=>({data:[]})),
   db.collection('user_actions').limit(2000).get()
  ]);
  const dishes=dishesRes.data||[], actions=actionsRes.data||[], eats=eatRes.data||[], favs=favRes.data||[], events=eventsRes.data||[], community=allActionsRes.data||[];
  ...
  let eventSignals=0;
  events.forEach(a=>{const d=dishMap.get(Number(a.dishId)),tags=tagsOf(d).length?tagsOf(d):arr(a.tags);const w=WEIGHTS[a.type]||0;if(w){const signed=a.type==='no'||a.type==='unfavorite'?-Math.abs(w):w;addAffinity(tags,signed*decay(a.createdAt));eventSignals++}if(a.type==='no')tags.forEach(t=>negativeTags.add(t))});
  ...
  const ranked=dishes.map(d=>{
   ...
   const tasteCount=Math.max(1,taste.length),profileScore=tasteCount<3?Math.min(82,direct/tasteCount*82):Math.min(100,(direct/tasteCount*70)+(Math.min(1,Math.abs(affNorm)/10)*30));
  ...
  const learnedTags=[...tagAffinity.entries()].sort((a,b)=>b[1]-a[1]).slice(0,8).map(x=>x[0]);
  return{success:true,data:result,profile:{taste:taste.slice(0,30),signals:actions.length+eats.length+favs.length,eventSignals,learnedTags,stage:eventSignals<10?'cold':eventSignals<50?'learning':'mature'}};
